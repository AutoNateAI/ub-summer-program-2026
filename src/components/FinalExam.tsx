...
import { finalExam } from "@/lib/finalExam";

type AssessmentSaveData = {
  assessmentId: string;
  name: string;
  email: string;
...
async function assessmentDocId(name: string, email: string) {
  const payload = `${finalExam.id}:${normalizeName(name)}:${normalizeEmail(email)}`;
...
function scoreAnswers(answers: Record<string, number>) {
  return finalExam.questions.reduce(
    (total, question) =>
...
export function FinalExam() {
  ...
  const allAnswered = answeredCount === finalExam.questions.length;
  const currentQuestion = finalExam.questions[currentIndex];
  const currentAnswer = answers[currentQuestion.id];
  const isLastQuestion = currentIndex === finalExam.questions.length - 1;
  const unlockAt = useMemo(() => new Date(finalExam.unlockAtIso), []);
  const locked = now.getTime() < unlockAt.getTime();
  ...
  async function loadProgress(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const id = await assessmentDocId(name, email);
      const ref = doc(db, "finalExamResponses", id);
      const snapshot = await getDoc(ref);

      if (snapshot.exists()) {
        const data = snapshot.data() as AssessmentSaveData;
        setAnswers(data.answers || {});
        setSubmitted(Boolean(data.submitted));
        setStatus("Loaded saved assessment progress.");
      } else {
        setAnswers({});
        setSubmitted(false);
        setStatus("No saved assessment found yet. Start below and save as you go.");
      }
  ...
  async function saveProgress(nextSubmitted = submitted) {
    ...
    setBusy(true);
    setStatus(nextSubmitted ? "Submitting assessment..." : "Saving progress...");
    ...
    try {
      ...
      await setDoc(
        ref,
        {
          ...
          answeredCount,
          score,
          totalPoints: finalExam.points,
          submitted: nextSubmitted,
          dueLabel: finalExam.dueLabel,
      ...
      setLoaded(true);
      setStatus(nextSubmitted ? "Assessment submitted and saved." : "Progress saved.");
    } catch (error) {
      console.error(error);
      ...
      >
        <div className="bg-[#071821] px-5 py-6 text-white sm:px-6">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[#c9d9df]">
            The exam opens on {unlockLabel}. Use the review guide and your team
            Google Doc to prepare before class.
          </p>
        </div>
        <div className="grid gap-4 p-5 sm:p-6 md:grid-cols-[220px_1fr]">
          <div className="grid place-items-center rounded-lg bg-[#e9f2f3] p-6 text-[#174b54]">
          ...
          <div className="rounded-lg border border-[#d4dbe0] bg-[#fbfaf7] p-5">
            ...
            <h4 className="mt-2 text-2xl font-black text-[#14384c]">
              Review first. The questions will appear when the exam unlocks.
            </h4>
            <p className="mt-3 text-sm leading-6 text-[#53616b]">
              This exam tests whether you can think through real scenarios:
              budget demand, department priorities, Chamber partners, program
              mechanics, app publishing, ChatGPT refinement, Base44 updates, and
              final opportunity proposals.
            </p>
          </div>
        </div>
      </section>
    ...
    >
      <div className="bg-[#071821] px-5 py-6 text-white sm:px-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.1em] text-[#ffd56a]">
              Final Exam · {finalExam.points} points
            </p>
            <h3 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">
              {finalExam.title}
            </h3>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#c9d9df]">
              This checks whether you can use the full course arc in real
              scenarios: budgets, departments, program research, Chamber
              partners, win-win-win design, Stitch, Base44, publishing,
              screenshots, AI tools, and opportunity proposals.
            </p>
          </div>
      ...
      <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[320px_1fr]">
        <aside className="h-fit rounded-lg border border-[#d4dbe0] bg-[#fbfaf7] p-4">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            {["Setup", "Identity", "Assessment"].map((step, index) => (
              <button
                key={step}
                type="button"
        ...
        <div className="min-h-[640px] overflow-hidden rounded-lg border border-[#d4dbe0] bg-[#fbfaf7]">
          {wizardStep === 0 ? (
            <div className="assessment-panel-enter grid gap-5 p-5 sm:p-7">
              <div className="rounded-lg bg-[#071821] p-5 text-white">
                ...
                <h4 className="mt-4 text-3xl font-black tracking-tight">
                  Before you start, know what this exam is testing.
                </h4>
                <p className="mt-3 max-w-2xl text-sm leading-6 text-[#c9d9df]">
                  This is not a memorization check. It asks whether you can
                  choose the right next move when a team is researching,
                  prompting, designing, publishing, refining, or pitching.
                </p>
              </div>

              <div className="grid gap-4 md:grid-cols-3">
                {[
                  ["1", "Read the system", "Budgets, departments, staff, and programs show where demand is becoming action."],
                  ["2", "Broker the offer", "Chamber partners, activities, ROI, and data turn the idea into a program."],
                  ["3", "Publish and refine", "Base44, screenshots, ChatGPT, and AI tools help improve the live software offer."],
          ...
          {wizardStep === 2 ? (
            <div className="grid gap-5 p-5 sm:p-7">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-black uppercase tracking-[0.08em] text-[#8f6417]">
                    Question {currentIndex + 1} of {finalExam.questions.length}
                  </p>
              ...
              <article
                ...
                className={`rounded-lg border border-[#d4dbe0] bg-white p-5 shadow-sm ${
                  slideDirection === "forward"
                    ? "assessment-slide-forward"
                    : "assessment-slide-back"
                }`}
              >
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="grid grid-cols-10 gap-1 sm:grid-cols-[repeat(15,minmax(0,1fr))] lg:w-[420px]">
