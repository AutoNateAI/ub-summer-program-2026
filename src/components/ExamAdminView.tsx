...
export function ExamAdminView() {
  ...
  async function loadResponses() {
    ...
    setStatus("Loading assessment responses from Firebase...");

    try {
      const responsesQuery = query(
        collection(db, "finalExamResponses"),
      ...
      setResponses(
        snapshot.docs.map((item) => ({
          id: item.id,
          ...(item.data() as Omit<AssessmentResponse, "id">),
        })),
      );
      if (!selectedResponseId && snapshot.docs[0]) {
        setSelectedResponseId(snapshot.docs[0].id);
      }
      setStatus("Assessment responses loaded.");
    } catch (error) {
      console.error(error);
      setStatus("Unable to load assessment responses. Check admin access and Firestore rules.");
    } finally {
      ...
      <header className="border-b border-[#d7d1c3] bg-[#071821] text-white">
        <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[#c9d9df]">
            ...
            for the final opportunity-to-app exam.
          </p>
        </div>
      </header>
        ...
        <>
          <section className="mx-auto grid max-w-7xl gap-4 px-5 pb-6 sm:px-8 lg:grid-cols-3">
            {[
              ...
              ["Average Score", `${averageScore.toFixed(1)}/${finalExam.points}`],
            ].map(([label, value]) => (
              <article
                key={label}
            ...
            <div className="mt-4 grid gap-5 xl:grid-cols-[420px_1fr]">
                    ...
                    >
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                        <div className="rounded-md bg-[#e9f2f3] px-2 py-2">
                          ...
                          <p className="text-sm font-black text-[#14384c]">
                            {response.score || 0}/{response.totalPoints || finalExam.points}
                          </p>
                        </div>
                        <div className="rounded-md bg-[#e9f2f3] px-2 py-2">
                          <p className="text-xs font-black uppercase tracking-[0.08em] text-[#65727b]">
                          ...
                          <p className="text-sm font-black text-[#14384c]">
                            {response.answeredCount || 0}/{finalExam.questions.length}
                          </p>
                        </div>
                        <div
              ...
              {!responses.length ? (
                <div className="rounded-lg border border-dashed border-[#c5ced5] bg-white p-5 text-sm font-bold text-[#65727b]">
                  No assessment responses have been saved yet.
                </div>
              ) : null}
                  ...
                  <div className="grid min-h-[640px] xl:grid-cols-[1fr_380px]">
                    <div className="p-5">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="rounded-md bg-[#e9f2f3] px-4 py-3 text-sm font-black text-[#174b54]">
                          ...
                          {selectedResponse.totalPoints || finalExam.points} ·{" "}
                    ...
                    <aside className="border-t border-[#d4dbe0] bg-[#fbfaf7] p-5 xl:border-l xl:border-t-0">
                      {selectedQuestionIndex === null ? (
                        ...
                        (() => {
                          const question = finalExam.questions[selectedQuestionIndex];
                          const selected =
                          ...
                          return (
                            <div className="assessment-panel-enter rounded-lg border border-[#d4dbe0] bg-white p-4">
